...
class RailManagement
  ...
  def create_train_or_wagon_dialogue(obj)
    ...
    puts "Введите номер(имя) #{nm}"
    name  = gets.chomp
    puts "Это пассажирский #{nm[0...-1]}? Введите 'да' или 'нет'"
    is_pas = gets.chomp
    if is_pas == "да"
      new =  is_train ? PassengerTrain.new(name) :  PassengerWaggon.new(name)
      ar << new
      puts "\nСоздан новый пассажирский #{nm[0...-1]} с номером #{new.name}\n"
    elsif is_pas == "нет"
      new =  is_train ? CargoTrain.new(name) : CargoWaggon.new(name)
      ar << new
      puts "\nСоздан новый грузовой #{nm[0...-1]} с номером #{new.name}\n"
    else

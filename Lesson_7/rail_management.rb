...
class RailManagement
  ...
  def create_train_or_wagon_dialogue(obj)
    ...
    begin
      ...
      if is_pas == "да"
        if is_train
          new = PassengerTrain.new(name, number)
        else
          puts "Введите общее количество пассажирских мест"
          seats = gets.chomp.to_i
          new = PassengerWaggon.new(name, seats)
        end
        ar[name] =  new
        puts "\nСоздан новый пассажирский #{nm[0...-1]} #{new.name}\n"
      elsif is_pas == "нет"
        if is_train
          new = CargoTrain.new(name, number)
        else
          puts "Введите общий объем грузового вагона"
          space = gets.chomp.to_f
          new =  CargoWaggon.new(name, space)
        end
        ar[name] =  new
